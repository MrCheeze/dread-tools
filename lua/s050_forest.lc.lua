...
function s050_forest.InitFromBlackboard()
  ...
  _UPVALUE0_ = Scenario.ReadFromBlackboard(Scenario.LUAPropIDs.FOREST_POSTXRELEASE_APPLIED, false)
...
s050_forest.tGetOnDeathOverrides = {ShowDeath = true, GoToMainMenu = false}
...
function s050_forest.SonarObtained()
  if Game.GetActor("powerup_sonar") ~= nil then
    Game.GetActor("powerup_sonar").PICKABLE:OnPickUpAfterCutScene()
  end
end
function s050_forest.OnEnter_trigger_LineBoomObtained(_ARG_0_, _ARG_1_)
  ...
  if Game.GetPlayer() ~= nil then
    Game.GetPlayer().INPUT:IgnoreInput(true, false, "OnEnter_trigger_LineBoomObtained")
  end
end
function s050_forest.DelayedLineBoomObtined()
  GUI.ShowMessage("#CUT_LINE_BOMB", true, "s050_forest.LineBoomObtainedMessageSkipped")
  if Game.GetPlayer() ~= nil then
    Game.GetPlayer().INPUT:IgnoreInput(false, false, "OnEnter_trigger_LineBoomObtained")
  end
end
...
function s050_forest.OnEnter_trigger_BossPresentation(_ARG_0_, _ARG_1_)
  ...
  if Game.GetPlayer() ~= nil then
    Game.GetPlayer().INPUT:IgnoreInput(true, false, "OnEnter_trigger_BossPresentation")
  end
end
function s050_forest.DelayedBossPresentation()
  ...
  if Game.GetPlayer() ~= nil then
    Game.GetPlayer().INPUT:IgnoreInput(false, false, "OnEnter_trigger_BossPresentation")
  end
end
...
function s050_forest.OnEnter_TG_camera_ZoomIn_On(_ARG_0_, _ARG_1_)
  ...
  if Game.GetActor("trigger_camera_ZoomIn_off") ~= nil then
    Game.GetActor("trigger_camera_ZoomIn_off").bEnabled = true
  end
end
...
function s050_forest.OnEnter_AP_07()
  if Game.GetActor("accesspoint_000") ~= nil then
    if Blackboard.GetProp("GAME_PROGRESS", "ADAMDIALOGUE") == "DIAG_ADAM_LAB_2" then
      Game.GetActor("accesspoint_000").USABLE:ActiveDialogue("DIAG_ADAM_FOREST_1")
      Blackboard.SetProp("GAME_PROGRESS", "ADAMDIALOGUE", "s", "DIAG_ADAM_FOREST_1")
    elseif Blackboard.GetProp("GAME_PROGRESS", "ADAMDIALOGUE") == "DIAG_ADAM_FOREST_1" and _UPVALUE0_ then
      Game.GetActor("accesspoint_000").USABLE:ActiveDialogue("DIAG_ADAM_FOREST_2")
...
function s050_forest.OnSubAreaChange(_ARG_0_, _ARG_1_, _ARG_2_, _ARG_3_, _ARG_4_)
  ...
  if _ARG_0_ == "collision_camera_002" and _ARG_1_ == "Default" and Game.GetActor("SG_ChozoRobotSoldier") ~= nil then
    print(Game.GetActor("SG_ChozoRobotSoldier").SPAWNGROUP.iNumDeaths)
    if Game.GetActor("SG_ChozoRobotSoldier").SPAWNGROUP.iNumDeaths > 0 then
      Game.PushSetup("PostChozoRobotSoldier", true, true)
    end
  end
  if _ARG_0_ == "collision_camera_023" and _ARG_1_ == "ChozoWarriorX" and Game.GetActor("SG_ChozoWarriorX") ~= nil then
    print(Game.GetActor("SG_ChozoWarriorX").SPAWNGROUP.iNumDeaths)
    if 0 < Game.GetActor("SG_ChozoWarriorX").SPAWNGROUP.iNumDeaths then
...
function s050_forest.LaunchCutscene_33()
  if Game.GetActor("cutsceneplayer_33") ~= nil then
    Game.GetActor("cutsceneplayer_33").CUTSCENE:TryLaunchCutscene()
  end
end
...
function s050_forest.DelayedOnEmmyForestDead()
  ...
  if Game.GetPlayer() ~= nil then
    Game.GetPlayer().INPUT:IgnoreInput(false, false, "OnEmmyForestDead")
  end
end
...
function s050_forest.OnEnter_PatrolEmmyActivator(_ARG_0_, _ARG_1_)
  s050_forest.ChangePatrolEmmy((string.gsub(_ARG_0_.sName, "TG_PATROLEMMYACTIVATOR_", "PATROLROUTE_")))
end
...
function s050_forest.OnWalkThroughEmmyDoor(_ARG_0_, _ARG_1_, _ARG_2_)
  if _UPVALUE0_ == true and Scenario.CheckEmmyAlive(CurrentScenario.oEmmyEntity) then
    if _ARG_1_ then
...
function s050_forest.Checkpoint_RelocatingEmmy_CU()
  if CurrentScenario.oEmmyEntity ~= nil and Game.GetActor("LM_EmmyEntrancePoint_017") ~= nil then
    CurrentScenario.oEmmyEntity.bEnabled = false
    CurrentScenario.oEmmyEntity.vPos = Game.GetActor("LM_EmmyEntrancePoint_017").vPos
    CurrentScenario.oEmmyEntity.vAng = Game.GetActor("LM_EmmyEntrancePoint_017").vAng
    CurrentScenario.oEmmyEntity.bEnabled = true
...
function s050_forest.OnEnter_Enable_ChozoWarriorX(_ARG_0_, _ARG_1_)
  ...
  if Game.GetActor("SG_PostDoubleJump") ~= nil then
    Game.GetActor("SG_PostDoubleJump").SPAWNGROUP:EnableSpawnGroup()
  end
end
...
function s050_forest.OnEnter_ActivateArenaSpawngroup(_ARG_0_, _ARG_1_)
  if Game.GetActor("SG_Gooplot_000") ~= nil then
    Game.GetActor("SG_Gooplot_000").SPAWNGROUP:EnableSpawnGroup()
  end
  _ARG_0_.bEnabled = false
end
function s050_forest.OnEnter_PostSuperGoliath(_ARG_0_, _ARG_1_)
  if Game.GetActor("SG_PostSuperGoliath_000") ~= nil then
    Game.GetActor("SG_PostSuperGoliath_000").SPAWNGROUP:EnableSpawnGroup()
  end
  _ARG_0_.bEnabled = false
end
...
function s050_forest.LockDoorSuperGoliathArena(_ARG_0_, _ARG_1_)
  if Game.GetActor("doorpowerclosed_003") ~= nil then
    Game.GetActor("doorpowerclosed_003").LIFE:LockDoor()
  end
  _ARG_0_.bEnabled = false
end
function s050_forest.OnEnter_Presentation_ChozoRobotSoldier(_ARG_0_, _ARG_1_)
  if Game.GetActor("SG_ChozoRobotSoldier") ~= nil then
    Game.GetActor("SG_ChozoRobotSoldier").SPAWNGROUP:EnableSpawnGroup()
  end
...
function s050_forest.CRS_DetectingDirection()
  ...
  if Game.GetActor("SP_ChozoRobotSoldier_chozorobotsoldier") ~= nil then
    if Game.GetPlayer() ~= nil then
    end
    if 0 < Game.GetPlayer().vPos - Game.GetActor("SP_ChozoRobotSoldier_chozorobotsoldier").vPos.x then
      print("----- TURN RIGHT -----")
      Game.GetActor("SP_ChozoRobotSoldier_chozorobotsoldier").ANIMATION:SetAction("spawn_front_turn_right", true)
    else
      print("----- TURN LEFT -----")
      Game.GetActor("SP_ChozoRobotSoldier_chozorobotsoldier").ANIMATION:SetAction("spawn_front_turn_left", true)
    end
  end
end
