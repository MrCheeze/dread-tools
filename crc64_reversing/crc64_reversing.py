...
def crc64(string, start_checksum=0xFFFFFFFFFFFFFFFF):
    if type(string) == str:
        string = bytes(string, encoding="ansi")
...
for i,x in enumerate(QWORD_ARRAY_71015f7b28):
    topbyte_lookup[x & 0xFF00000000000000] = (i,x)
assert len(topbyte_lookup) == 256

def remove_suffix(checksum, suffix):
    for suffix_char in reversed(suffix):
        i,x = topbyte_lookup[checksum & 0xFF00000000000000]
        checksum = ((checksum ^ x) << 8) ^ (i ^ ord(suffix_char))
    return checksum
...
for goal_checksum in [crc64('vibrations/weapons/powerbomb_03.bnvib')]:
    ...
    suffix = '.bnvib'
    noextension_checksum = remove_suffix(goal_checksum, suffix)
    ...
    mask = 0
    strs = []
    for i in range(8 - rewind_count):
        mask <<= 8
        mask |= 0xFF
    for i in range(rewind_count):
        mask <<= 8
        mask |= 0x80
        strs.append('?'*(i+1))
    ...
    for a in chars:
        print(a)
        checksum_a = remove_suffix(noextension_checksum, a)
        nosuffix_checksums[checksum_a & mask] = a
        for b in chars:
            checksum_b = remove_suffix(checksum_a, b)
            nosuffix_checksums[checksum_b & mask] = b+a
            for c in chars:
                checksum_c = remove_suffix(checksum_b, c)
                nosuffix_checksums[checksum_c & mask] = c+b+a
                for d in chars:
                    checksum_d = remove_suffix(checksum_c, d)
                    nosuffix_checksums[checksum_d & mask] = d+c+b+a
                    for e in chars:
                        checksum_e = remove_suffix(checksum_d, e)
                        nosuffix_checksums[checksum_e & mask] = e+d+c+b+a
                        
                        for i in range(rewind_count):
                            checksum_e = remove_suffix(checksum_e, '\x00')
                            nosuffix_checksums[checksum_e & mask] = strs[i]+e+d+c+b+a

    #def test(s):
    #    print(s)
    #    if '?' in s:
    #        for char in chars:
    #            test(s.replace('?',char,1))
    #    else:
    #        if crc64(prefix+s+suffix) == goal_checksum:
    ...
    for a in chars:
        print(a)
        checksum_a = crc64(a, start_checksum=prefix_checksum)
        if checksum_a & mask in nosuffix_checksums:
            s = a
            print(s+nosuffix_checksums[checksum_a & mask])
        for b in chars:
            checksum_b = crc64(b, start_checksum=checksum_a)
            if checksum_b & mask in nosuffix_checksums:
                s = a+b
                print(s+nosuffix_checksums[checksum_b & mask])
            for c in chars:
                checksum_c = crc64(c, start_checksum=checksum_b)
                if checksum_c & mask in nosuffix_checksums:
                    s = a+b+c
                    print(s+nosuffix_checksums[checksum_c & mask])
                for d in chars:
                    checksum_d = crc64(d, start_checksum=checksum_c)
                    if checksum_d & mask in nosuffix_checksums:
                        s = a+b+c+d
                        print(s+nosuffix_checksums[checksum_d & mask])
                    for e in chars:
                        checksum_e = crc64(e, start_checksum=checksum_d)
                        if checksum_e & mask in nosuffix_checksums:
                            s = a+b+c+d+e
                            print(s+nosuffix_checksums[checksum_e & mask])
                        #for f in chars:
                        #    checksum_f = crc64(f, start_checksum=checksum_e)
                        #    if checksum_f & mask in nosuffix_checksums:
                        #        s = a+b+c+d+e+f
                        #        print(s+nosuffix_checksums[checksum_f & mask])
